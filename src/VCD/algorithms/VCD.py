...
class VerticalCellDecomposition:
    ...
    def region_disection(self):
        ...
        if(len(self.decomposition_lines_map[self.polygon_vertices[0]]) == 2):
            firstCell = [self.cspace.boundary[3],self.cspace.boundary[3],self.decomposition_lines_map[self.polygon_vertices[0]][0][1],\
                 self.decomposition_lines_map[self.polygon_vertices[0]][1][1]]
            self.regions.append(firstCell)
             
        if(len(self.decomposition_lines_map[self.polygon_vertices[-1]]) == 2):
