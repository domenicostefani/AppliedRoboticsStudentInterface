...
WindowData displayControl(const cv::Mat& frame, string windowName,
  ...
  /*--------------------------------------------------------------------------*/
  createTrackbar("Low H" , windowName, &wd.low_h , 180,  on_low_h_thresh_change,
                 (void *)&wd);
  createTrackbar("High H", windowName, &wd.high_h, 180, on_high_h_thresh_change,
                 (void *)&wd);
  createTrackbar("Low S" , windowName, &wd.low_s , 255,  on_low_s_thresh_change,
                 (void *)&wd);
  createTrackbar("High S", windowName, &wd.high_s, 255, on_high_s_thresh_change,
                 (void *)&wd);
  createTrackbar("Low V" , windowName, &wd.low_v , 255,  on_low_v_thresh_change,
                 (void *)&wd);
  createTrackbar("High V", windowName, &wd.high_v, 255, on_high_v_thresh_change,
                 (void *)&wd);
  ...
  while(((char)waitKey(1)!='n') && (keepopen == true)){
      ...
      inRange(frame, Scalar(wd.low_h,wd.low_s,wd.low_v),
              Scalar(wd.high_h,wd.high_s,wd.high_v), frame_threshold);
      //-- Show the frames
